...
func main() {
	...
	camera := internal.NewCamera(
		...
		internal.WithLookFrom(internal.NewVec3(13, 2, 3, 0)),
		internal.WithLookAt(internal.NewVec3(0, 0, 0, 0)),
	...
	matGround := internal.NewLambertian(internal.NewVec3(0.5, 0.5, 0.5, 0))

	hittables := []internal.Hittable{
		&internal.Sphere{
			Center:   internal.NewVec3(0, -1000, 0, 0),
			Radius:   1000,
			Material: &matGround,
		},
	}
	...
	p := internal.NewVec3(4, 0.2, 0, 0)
	for i := -11; i < 11; i++ {
		for j := -11; j < 11; j++ {
			matPer := rand.Float32()
			center := internal.NewVec3(float32(i)+0.9*rand.Float32(), 0.2, float32(j)+0.9*rand.Float32(), 0)
	...
	hittables = append(hittables, &internal.Sphere{
		Center:   internal.NewVec3(0, 1, 0, 0),
		Radius:   1,
		Material: &m1,
	})
	m2 := internal.NewLambertian(internal.NewVec3(0.4, 0.2, 0.1, 0))
	hittables = append(hittables, &internal.Sphere{
		Center:   internal.NewVec3(-4, 1, 0, 0),
		Radius:   1,
		Material: &m2,
	})
	m3 := internal.NewMetal(internal.NewVec3(0.7, 0.6, 0.5, 0), 0)
	hittables = append(hittables, &internal.Sphere{
		Center:   internal.NewVec3(4, 1, 0, 0),
		Radius:   1,
		Material: &m3,
